...
export const getExpensesBetweenUsers = query({
  ...
  handler: async (ctx, { userId }) => {
    ...
    const currentUserPaid = await ctx.db
      .query("expenses")
    ...
    const currentUserNotPaid = await ctx.db
      .query("expenses")
      .withIndex("by_user_and_group", (q) =>
    ...
    const candidateExpenses = [...currentUserPaid, ...currentUserNotPaid];

    /* 2. Keep only rows where BOTH are involved (payer or split) */
    const expenses = candidateExpenses.filter((e) => {
      // currentUser is always involved (I’m the payer OR in splits – verified below)
      const currentUserInSplits = e.splits.some(
        (s) => s.userId === currentUser._id
      );
      const themInSplits = e.splits.some((s) => s.userId === userId);

      const currentUserInvolved =
        e.paidByUserId === currentUser._id || currentUserInSplits;
      const themInvolved = e.paidByUserId === userId || themInSplits;

      return currentUserInvolved && themInvolved;
    });
    ...
    for (const e of expenses) {
      if (e.paidByUserId === currentUser._id) {
        ...
        if (split) balance += split.amount; // they owe currentUser
      } else {
    ...
    for (const s of settlements) {
      ...
      else balance -= s.amount; // they paid currentUser back
    }
